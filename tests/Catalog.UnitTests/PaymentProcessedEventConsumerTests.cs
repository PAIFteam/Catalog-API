...
public class PaymentProcessedEventConsumerTests
{
    ...
    [Fact]
    public async Task Consume_PagamentoAprovado_DeveExecutarConcessaoDoJogo()
    {
        var repository = new Mock<IAddGameUserRepository>();
        repository.Setup(x => x.AddGameUserAsync(It.IsAny<Catalog.Core.Domain.Entities.GameUser>())).ReturnsAsync(true);
        var addGameUserUseCase = new AddGameUserUseCase(repository.Object, Mock.Of<ILogger<AddGameUserUseCase>>());
        var context = new Mock<ConsumeContext<PaymentProcessedMessage>>();
        context.SetupGet(x => x.Message).Returns(new PaymentProcessedMessage(1, 2, 10m, true, "aprovado"));
        var sut = new PaymentProcessedEventConsumer(addGameUserUseCase, Mock.Of<ILogger<PaymentProcessedEventConsumer>>());

        await sut.Consume(context.Object);

        repository.Verify(x => x.AddGameUserAsync(It.Is<Catalog.Core.Domain.Entities.GameUser>(g => g.IdUser == 1 && g.IdGame == 2)), Times.Once);
    }
}
